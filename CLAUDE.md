...
五大核心模块(资产管理、审批工作流、报表统计、RBAC/基础数据、**新产品新技术(测试料件)**)已全面打通。截至 2026-07-22，后端测试集共 **519 项**、前端单元测试共 **476 项**。本次部门主管分类维护与跨部门普通员工增删改查变更已通过后端完整回归(519/519)、前端全部单元测试(476/476)及 `web-ele` 类型检查。后端集成测试要求提供独立 MySQL 测试库配置。
